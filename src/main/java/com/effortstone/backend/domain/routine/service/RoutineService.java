...
public class RoutineService {
    ...
    // 🔹 루틴 생성 (Builder 적용)
    public Routine createRoutine(RoutineRequestDto.RoutineCreateRequest routine) {
        String userCode = SecurityUtil.getCurrentUserCode();
        ...
        Routine newRoutine = Routine.builder()
                ...
                .routineGoalType(routine.getRoutineGoalType())
    ...
    // 🔹 루틴 수정 (Builder 적용)
    public Routine updateRoutine(Long routineCode, Routine routineDetails) {

        String userCode = SecurityUtil.getCurrentUserCode();
        Routine existingRoutine = getRoutineById(routineCode);

        // 🔹 사용자 검증 추가
        if (!existingRoutine.getUser().getUserCode().equals(userCode)) {
            throw new RuntimeException("You do not have permission to update this routine.");
        }

        Routine updatedRoutine = Routine.builder()
                .routineCode(existingRoutine.getRoutineCode())  // 기존 ID 유지
                .user(existingRoutine.getUser())  // 기존 User 유지
                .routineName(routineDetails.getRoutineName())
                .routineGoalType(routineDetails.getRoutineGoalType())
                .routineFocusTime(routineDetails.getRoutineFocusTime())
                .routineRepeatFrequency(routineDetails.getRoutineRepeatFrequency())
                .routineStartDate(routineDetails.getRoutineStartDate())
                .routineEndDate(routineDetails.getRoutineEndDate())
                .routineTheme(routineDetails.getRoutineTheme())
                .routineDetail(routineDetails.getRoutineDetail())
                .routineStartTime(routineDetails.getRoutineStartTime())
                .routineEndTime(routineDetails.getRoutineEndTime())
                .routineAlertTime(routineDetails.getRoutineAlertTime())
                .build();
